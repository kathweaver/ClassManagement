...
function openpopup(popurl){
	winpops=window.open(popurl,"","width=350,height=250, scrollbars, resizable")
	winpops=self.close();
	}
</script>
<body> 
...
foreach($weeks AS $week){ 
       ...
       foreach($week as $d){ 
         if($i < $offset_count){ 
		     $dayurl = "test1.php?date=".mktime(0,0,0,$month -1,$d,$year);
		     $day_link = "<a href=\"javascript:openpopup('".$dayurl."')\">$d</a>"; 
             echo "<td class=\"nonmonthdays\">$day_link</td>\n"; 
         } 
         if(($i >= $offset_count) && ($i < ($num_weeks * 7) - $outset)){ 
 		     $dayurl = "test1.php?date=".mktime(0,0,0,$month ,$d,$year);
		     $day_link = "<a href=\"javascript:openpopup('".$dayurl."')\">$d</a>";
