...
include 'noheader.php';
		...
		$sqlstring = "SELECT * FROM submissions sub, users user WHERE grade IS NOT NULL AND recorded = '1' AND gradespeed = '0' AND sub.userid = user.userid ORDER BY user.period, user.class, sub.assignment_name, user.last_name";
		...
		echo ("<tr>");
	    echo("<td><a href='http://www.kweaver.net/class/submissions/record_assign_gradespeed.php?index=$index'>$index</a></td><td>$period</td><td>$class</td><td>$first_name</td><td>$last_name</td><td>$assignment_name</td><td>$submission_time</td><td>$grade</td><td>$grade_comments</td><TD><a href='http://www.kweaver.net/class/submissions/grade_assign.php?index=$index'>$index</a></TD></tr>");
		} //end while loop
	echo "</table></center>";
	}
